...
export async function GET(request: NextRequest) {
  try {
    ...
    const status = searchParams.get('status') // 'all', 'replied', 'pending'
    ...
    const stats = await Review.aggregate([
      { $match: { sellerId: decoded.userId, isHidden: false } },
      {
        $group: {
          _id: null,
          totalReviews: { $sum: 1 },
    ...
    const summary = stats.length > 0 ? {
      ...
      repliedCount: stats[0].repliedCount,
      pendingCount: stats[0].pendingCount,
      replyRate: stats[0].totalReviews > 0 ? Math.round((stats[0].repliedCount / stats[0].totalReviews) * 100) : 0,
      distribution
    } : {
      totalReviews: 0,
      averageRating: 0,
