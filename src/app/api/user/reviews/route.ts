...
// GET /api/user/reviews - Get reviews written by any user (all roles)
export async function GET(request: NextRequest) {
  try {
    ...
    const summaryStats = await Review.aggregate([
      { $match: { userId: decoded.userId, isHidden: false } },
      {
        $group: {
          _id: null,
          totalReviews: { $sum: 1 },
    ...
    const summary = summaryStats.length > 0 ? {
      totalReviews: summaryStats[0].totalReviews,
      averageRating: Math.round(summaryStats[0].averageRating * 10) / 10,
      repliesReceived: summaryStats[0].repliesReceived,
      replyRate: summaryStats[0].totalReviews > 0 ? Math.round((summaryStats[0].repliesReceived / summaryStats[0].totalReviews) * 100) : 0,
      distribution
    } : {
      totalReviews: 0,
      averageRating: 0,
